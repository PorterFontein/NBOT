(() => {
        ...
        function animOnScroll(params) {
            for (let i = 0; i < animItems.length; i++) {
                ...
                if (pageYOffset > animItemOffset - animItemPoint && pageYOffset < animItemOffset + animItemHeight) {
                    ...
                    if (i == 2) {
                        const circle = document.getElementById("circleA");
                        circle.style.animation = "pieThree 1.5s linear forwards";
                        circle.style.animationDelay = ".5s";
                    }
                    if (i == 3) {
                        const circle = document.getElementById("circleE");
                        circle.style.animation = "pieFour 1.5s linear forwards";
                        circle.style.animationDelay = ".5s";
                    }
                    if (flag != 2) numberCount();
                }
            }
        }
        function offset(el) {
    ...
    function numberCount() {
        flag++;
        let number = document.querySelectorAll(".develop__number");
        for (let i = 0; i < number.length; i++) if (i == 0) setTimeout((() => {
            drawPie(number[i], 45, 20);
        }), 400); else if (i == 1) setTimeout((() => {
            drawPie(number[i], 70, 10);
        }), 400); else if (i == 2) setTimeout((() => {
            drawPie(number[i], 30, 20);
        }), 400); else if (i == 3) setTimeout((() => {
            drawPie(number[i], 80, 10);
        }), 400);
    }
    function drawPie(number, stop, time) {
        let counter = 0;
        setInterval((() => {
            if (counter === stop) clearInterval(); else {
                counter++;
